...
export default function SignUpPage(): JSX.Element {
  ...
  useEffect(() => {
    ...
    console.log('🔍 [DEBUG] SignUpPage: Search params:', window.location.search)

    // Check if we're coming from a redirect
    const urlParams = new URLSearchParams(window.location.search)
    if (urlParams.has('redirect_url')) {
      console.log('🔍 [DEBUG] SignUpPage: Redirect URL detected:', urlParams.get('redirect_url'))
    }
  ...
  useEffect(() => {
    console.log('🔍 [DEBUG] SignUpPage: Username modal state changed:', showUsernameModal)
  }, [showUsernameModal])
