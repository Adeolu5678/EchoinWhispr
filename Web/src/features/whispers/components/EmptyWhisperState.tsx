...
export const EmptyWhisperState: React.FC<EmptyWhisperStateProps> = React.memo(
  ({ message, actionLabel, onAction, className = '' }) => {
    const defaultMessage =
      "You haven't received any whispers yet. When someone sends you a message, it will appear here.";
    const defaultActionLabel = 'Send Your First Whisper';

    return (
      <Card
        className={`border-dashed border-2 ${className}`}
        role="region"
        aria-label="No whispers"
      >
          ...
          <div className="space-y-4">
            ...
            <div className="pt-4 border-t border-border/50">
              <p className="text-sm text-muted-foreground">
                💭 Whispers are anonymous messages that can brighten
                someone&apos;s day
              </p>
            </div>
          </div>
