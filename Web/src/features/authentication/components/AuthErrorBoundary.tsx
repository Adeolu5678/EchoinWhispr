...
export class AuthErrorBoundary extends Component<AuthErrorBoundaryProps, AuthErrorBoundaryState> {
  ...
  private errorReportQueue: Array<{ error: Error; errorInfo: ErrorInfo; timestamp: number }> = []
  ...
  private categorizeError(error: Error): string {
    ...
    if (errorMessage.includes('config') || errorMessage.includes('setup') || errorMessage.includes('initialization')) {
  ...
  private isRecoverableError(error: Error): boolean {
    ...
    const recoverableCategories = ['NETWORK_ERROR', 'RATE_LIMIT_ERROR', 'CLERK_SDK_ERROR']
  ...
  private handleRetry = () => {
    ...
    console.log(`🔄 Retrying authentication (attempt ${retryCount + 1}/${maxRetries}) in ${Math.round(delay)}ms...`)
  ...
  private showMaxRetriesToast = () => {
    ...
    import('@/hooks/use-toast').then(({ useToast }) => {
      ...
      toast({
        ...
        description: 'Unable to recover from this error. Please try refreshing the page or contact support.',
        variant: 'destructive',
      })
    })
  }
  ...
  private showNonRecoverableErrorToast = () => {
    ...
    import('@/hooks/use-toast').then(({ useToast }) => {
      ...
      toast({
        ...
        description: 'This error cannot be automatically resolved. Please refresh the page or contact support.',
        variant: 'destructive',
      })
    })
  }
  ...
  private getErrorMessage = (): { title: string; description: string } => {
    ...
    switch (category) {
      ...
      case 'AUTHENTICATION_ERROR':
        return {
          title: 'Authentication Error',
          description: 'There was a problem with your authentication session. Please try signing in again.',
        }
      case 'CONFIGURATION_ERROR':
        return {
          title: 'Configuration Error',
          description: 'There seems to be a configuration issue with the authentication system.',
        }
      ...
      case 'CLERK_SDK_ERROR':
        return {
          ...
          description: 'The authentication service encountered an unexpected error. Please try again.',
        }
      case 'CONVEX_ERROR':
        return {
          title: 'Data Service Error',
          description: 'There was an issue connecting to our data service. Please try again.',
        }
      default:
        return {
          title: 'Authentication Error',
  ...
  render() {
    const { hasError, error, retryCount, showTechnicalDetails, errorId } = this.state
          ...
          <Card className="w-full max-w-lg" role="alert">
            ...
            <CardContent className="space-y-4">
              <div className="flex flex-col gap-2">
                {isRecoverable && retryCount < maxRetries && (
                  ...
                  >
                    ...
                    {retryCount === 0 ? 'Try Again' : `Try Again (${retryCount}/${maxRetries})`}
                  </Button>
                )}

                {retryCount >= maxRetries && (
                  <p className="text-sm text-gray-500 text-center">
                    Maximum retry attempts reached. Please try other options below.
                  </p>
                )}
              ...
              <div className="border-t pt-4">
                ...
                {showTechnicalDetails && (
                  <div id="auth-error-technical-details" className="mt-2 rounded-md bg-gray-100 p-3">
                    <div className="text-xs text-gray-700 space-y-3">
                      <div>
                        <div className="font-medium mb-1">Error Category:</div>
                        <div className="font-mono text-blue-600">{category}</div>
                      </div>
